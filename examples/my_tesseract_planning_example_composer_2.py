...
from tesseract_robotics.tesseract_command_language import CartesianWaypoint, WaypointPoly, \
    ...
    CompositeInstruction, MoveInstructionPoly, CartesianWaypointPoly, ProfileDictionary, \
        ...
        AnyPoly_wrap_ProfileDictionary, WaitInstruction, InstructionPoly_wrap_WaitInstruction, WaitInstructionType_TIME, \
...
# Retract waypoint: same as wp2 (pre-approach)
wp4 = CartesianWaypoint(wp2_pose)

# Drop-off waypoint: Move to the side
wp5 = CartesianWaypoint(
    axis_angle_pose(
        0.55,
        -0.20,
        0.30,
...
# Wait for 2 seconds
wait_instr = WaitInstruction(2.0)
# Retract linearly
plan_f3 = MoveInstruction(CartesianWaypointPoly_wrap_CartesianWaypoint(wp4), MoveInstructionType_LINEAR, "DEFAULT")
# Move to drop-off
plan_f4 = MoveInstruction(CartesianWaypointPoly_wrap_CartesianWaypoint(wp5), MoveInstructionType_FREESPACE, "DEFAULT")
...
# Create program_approach (Start -> Approach -> Pick)
program_approach = CompositeInstruction("DEFAULT")
program_approach.setManipulatorInfo(manip_info)
program_approach.appendMoveInstruction(MoveInstructionPoly_wrap_MoveInstruction(start_instruction))
program_approach.appendMoveInstruction(MoveInstructionPoly_wrap_MoveInstruction(plan_f1))
program_approach.appendMoveInstruction(MoveInstructionPoly_wrap_MoveInstruction(plan_f2)) # Pick (wp3)

# Create program_depart (Pick -> Retract -> Drop-off)
# Note: Should start from Pick (wp3)
program_depart = CompositeInstruction("DEFAULT")
program_depart.setManipulatorInfo(manip_info)
# We need a start instruction for the second trajectory. Using wp3 as start.
start_depart = MoveInstruction(CartesianWaypointPoly_wrap_CartesianWaypoint(wp3), MoveInstructionType_FREESPACE, "DEFAULT")
program_depart.appendMoveInstruction(MoveInstructionPoly_wrap_MoveInstruction(start_depart))
program_depart.appendMoveInstruction(MoveInstructionPoly_wrap_MoveInstruction(plan_f3))
program_depart.appendMoveInstruction(MoveInstructionPoly_wrap_MoveInstruction(plan_f4))


# Function to plan and execute a program
def run_planning(program_name, program, output_filename):
    print(f"Planning {program_name}...")
    ...
    if not future.context.isSuccessful():
        print(f"Planning {program_name} failed")
        return None
    ...
    if len(velocities) > 0 and len(velocities[0]) > 0:
        ...
        fixed_output_path = os.path.join(script_dir, output_filename)
        with open(fixed_output_path, "w", encoding="utf-8") as f:
            for t, pos in zip(fixed_timestamps, interpolated_positions):
                ...
                f.write(f"{pos_str}\n")
        print(f"Saved {program_name} to {fixed_output_path}")
        return results
    else:
        print(f"No velocities for {program_name}")
        return None

# Run both plans
results_approach = run_planning("Approach", program_approach, "joint_positions_approach.txt")
results_depart = run_planning("Depart", program_depart, "joint_positions_depart.txt")

# Update viewer with both (if successful)
# We can display them sequentially or just the last one, or combine them for visualization
if results_approach and results_depart:
    # Combine for viewer
    full_trajectory = CompositeInstruction("DEFAULT")
    full_trajectory.setManipulatorInfo(manip_info)
    
    # Add approach
    for instr in results_approach:
         if instr.isMoveInstruction():
             full_trajectory.append(instr)
             
    # Add depart (careful about time continuity if we were simulating, but for viewer it might just play sequentially)
    # The viewer might reset time for the second part or play it weirdly.
    # Ideally we'd shift times for the second part, but for simple visualization appending might be enough.
    for instr in results_depart:
        if instr.isMoveInstruction():
            full_trajectory.append(instr)

    viewer.update_trajectory(full_trajectory)
    viewer.plot_trajectory(full_trajectory, manip_info)
