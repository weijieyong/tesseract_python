...
class RobotArmController:
    ...
    def demo_movej_canfd(self):
        """
        Demonstrates Canfd_movej pass-through for 6 DOF arms only.
        ...
        2. Check the degree of freedom (DOF) of the robotic arm. # removed
        ...
        Reads joint positions from './examples/joint_positions_fixed_dt.txt' and moves the arm through these points.
        """
        try:
            info_result = self.robot.rm_get_robot_info()
            print(f"Robot Info: {info_result}")
            if info_result[1]['arm_model'] not in ('RM_65', 'RM_63'):
                raise ValueError('Only 6 DOF RM_65 or RM_63 models are supported in this simplified script')

            # Read file contents, the points in the file are obtained by drag teaching
            with open('./examples/joint_positions_fixed_dt.txt', 'r') as f:
                lines = f.readlines()
            ...
            if robot_info[1]['arm_dof'] != 6:
                raise ValueError("This script only supports 6 DOF arms.")
            dof = 6
            ...
            self.robot.rm_movej(points[0], 10, 0, rm_trajectory_connect_config_e.RM_TRAJECTORY_DISCONNECT_E, RM_MOVE_MULTI_BLOCK)
            ...
            time.sleep(2)
            movej_result = self.robot.rm_movej(points[0], 21, 0, rm_trajectory_connect_config_e.RM_TRAJECTORY_DISCONNECT_E, RM_MOVE_MULTI_BLOCK)
